  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        ...
        // Laravel resources biasanya mengembalikan objek { data: [...] }
        if (data.data && Array.isArray(data.data)) {
          setCourts(data.data);
        } else if (Array.isArray(data)) {
          setCourts(data);
        } else {
          throw new Error('Format data tidak sesuai ekspektasi (bukan array)');
        }
      } catch (error) {
        console.error("Error fetching fields:", error);
        // Fallback ke data dummy sementara jika API gagal/belum siap
        setCourts([
    ...
    <section className="max-w-7xl mx-auto px-6 py-24">
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20">
          ...
          <span className="ml-3 text-[#1B3627] font-medium">Memuat data lapangan...</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {courts.map((court, index) => (
            <div key={index} className="relative group overflow-hidden rounded bg-[#1B3627] text-white aspect-[4/5] shadow-lg">
              <img src={court.img || court.image} alt={court.name} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition duration-500" />
              ...
              <div className="absolute bottom-0 left-0 p-6 w-full">
                ...
                <p className="text-xs text-gray-300 mb-4">{court.type || "Fasilitas Olahraga"}</p>
